...
| Action | params | 说明 |
|:---|:---|:---|
| `recommend` | `{query, top_k?}` | 技能推荐 |
| `record` | `{skill, input, accepted?}` | 记录使用 |
| `refresh` | `{}` | 刷新索引 |
| `stats` | `{}` | 获取统计 |
| `coverage` | `{}` | 覆盖率分析 |
| `ping` | `{}` | 心跳检测 |
...
| `sra config [get/set]` | 配置管理 | `sra config set http_port 9000` |
...
|:--:|:---|:---|
| 200 | 成功 | 正常响应 |
| 400 | 参数错误 | 缺少 `message`/`query`、参数格式错误 |
| 404 | 端点不存在 | 访问未知路径 |
| 500 | 服务器内部错误 | 未捕获的异常 |
| 503 | 服务不可用 | 守护进程未启动 |
...
> **本文件是 SRA 的完整 API 参考。所有端点和参数以实际代码为准，文档与代码不一致时以代码为准。**
